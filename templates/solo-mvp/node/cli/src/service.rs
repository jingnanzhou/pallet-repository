...
pub fn create_extrinsic(
	...
	function: impl Into<mvp_runtime::RuntimeCall>,
	nonce: Option<u32>,
	...
	let period = mvp_runtime::BlockHashCount::get()
		.checked_next_power_of_two()
		.map(|c| c / 2)
		.unwrap_or(2) as u64;
	let tip = 0;
	let extra: mvp_runtime::SignedExtra = (
		frame_system::CheckNonZeroSender::<mvp_runtime::Runtime>::new(),
		frame_system::CheckSpecVersion::<mvp_runtime::Runtime>::new(),
		frame_system::CheckTxVersion::<mvp_runtime::Runtime>::new(),
		frame_system::CheckGenesis::<mvp_runtime::Runtime>::new(),
		frame_system::CheckEra::<mvp_runtime::Runtime>::from(generic::Era::mortal(
			period,
			best_block.saturated_into(),
		)),
		frame_system::CheckNonce::<mvp_runtime::Runtime>::from(nonce),
		frame_system::CheckWeight::<mvp_runtime::Runtime>::new(),
		pallet_asset_tx_payment::ChargeAssetTxPayment::<mvp_runtime::Runtime>::from(
			tip, None,
		),
	);

	let raw_payload = mvp_runtime::SignedPayload::from_raw(
		function.clone(),
		extra.clone(),
		(
			(),
			mvp_runtime::VERSION.spec_version,
			mvp_runtime::VERSION.transaction_version,
			genesis_hash,
			best_hash,
			(),
			(),
	...
	mvp_runtime::UncheckedExtrinsic::new_signed(
		function,
		sp_runtime::AccountId32::from(sender.public()).into(),
		mvp_runtime::Signature::Sr25519(signature),
		extra,
	)
}
